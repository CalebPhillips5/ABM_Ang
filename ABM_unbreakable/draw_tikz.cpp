...
int main ( int argc, char *argv[] ){
  ...
  output << "\\draw (axis cs:" << W/2 << "," << H/2 << ") circle [radius=" << H/2 << "];" << endl;
  for(int i = 0; i < S; i++){
    int state;
    ...
    output << "\\node at (axis cs:" << x << "," << y << ")[align = center,scale = 0.1] {\\bf " << i << "};" << endl;
    output << "\\node at (axis cs:" << x << "," << y-3.5 << ")[align = center,scale = 0.1] {$\\cdot$};" << endl;
  }
